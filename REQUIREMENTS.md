...
- **REQ-PNPM11-002** — `forceMinimumReleaseAgeZero` SHALL temporarily set
  `minimumReleaseAge: 0` in `pnpm-workspace.yaml` for the duration of the
  run.
- **REQ-PNPM11-003** — `restoreMinimumReleaseAge` SHALL restore the user’s
  original `minimumReleaseAge` value (or absence of the key) before the
  tool exits.
